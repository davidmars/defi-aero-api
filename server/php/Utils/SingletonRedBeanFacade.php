...
use EE09\Models\Ee09bean;
use Models\Tilty\TiltyBean;
...
/**
 * RedBean Facade HAAAACCKKKK....en mode sigleton
 *
 * Version Information
 ...
 *
 *
 * @method static SingletonRedBeanFacade inst()
 * @property PovRedBeanUtils $utils Utilities methods
 *
 */
class SingletonRedBeanFacade extends AbstractSingleton
{
    ...
    /**
     * Renvoie le record qui correspond à cet uid
     * @param string $uid
     * @param string $strictType
     * @return false|null|TiltyBean
     */
    public function getByUid($uid,$strictType=""){
        if(!$uid){
            return false;
        }
        if(preg_match("/^db:([a-zA-Z0-9]+)@([0-9]+)$/",$uid,$m)){
            $type=$m[1];
            $id=$m[2];
            if($strictType && $type !== $strictType){
                return false;
            }
            $record=$this->findOne($type,"where id = '$id'");
            if($record){
                return $record->box();
            }
        }
    ...
    /**
     * Renvoie la liste des records qui correspondent à ces uids
    ...
    public function getByUids($uids){
        $r=[];
        if($uids){
            $uids=utils()->array->fromString($uids);
            foreach ($uids as $uid){
                $record=$this->getByUid($uid);
                if($record){
                    $r[]=$record;
                }
            }
    ...
    {
        throw new \Exception("Redbean facade pas implémenté");
    }
